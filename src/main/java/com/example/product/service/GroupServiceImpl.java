	...
	@Autowired
		...
		public void createANewGroup(Groups newgroup) throws UserNotFoundException
		{
		    ...
		    if (adminUserOptional.isEmpty()) {
		        throw new UserNotFoundException("The admin with ID " + adminUserID + " doesn't exist. Therefore, the group cannot be created.");
		    }
		...
		@Override
		public void updateAnExistingGroup(Groups existingGroup, String updatedGroupName)
				throws GroupsNotFoundException, UserNotFoundException {
			 if (existingGroup == null) {
			        throw new GroupsNotFoundException("Existing group is null.");
			    }
	 
			    // Check if the users field is not null
			    if (existingGroup.getAdmin() == null) {
			        throw new UserNotFoundException("Users associated with the existing group is null.");
			    }
	 
			    // Check if the admin user associated with the existing group exists
			    int adminUserID = existingGroup.getAdmin().getUserID();
			    Optional<Users> adminUserOptional = usersRepository.findById(adminUserID);
			    ...
			    // Set the updated group name
			    existingGroup.setGroupName(updatedGroupName);
